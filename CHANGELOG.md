...
- Jujutsuリポジトリでの対話的ファイル選択機能を追加。通常モード（`gcauto`）実行時に、commit対象ファイルを番号トグル・全選択・全解除で柔軟に選択可能に
- Jujutsuでの部分commit機能を実装。選択されなかったファイルの変更を一時退避し、commit後に自動復元することで、staging areaのないJujutsuでもファイル単位のcommitを実現
- ファイル選択UIでの割り込み（SIGINT）時に、除外ファイルの復元処理を安全に行うエラーハンドリングを追加
- Jujutsu対話的ファイル選択機能に関するテストケースを追加（パーサー、ファイル選択UI、部分commit、パス指定diff取得）
